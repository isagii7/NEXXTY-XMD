...
// ========== 🔥 NEXXTY CONFIGS ==========
// ✅ آپ کا چینل انوائٹ کوڈ (مستقل)
const CHANNEL_INVITE = '0029Vb8mDiBCHDytzXwk1o0K';

// ✅ پہلے سے موجود JIDs (بیک اپ کے لیے)
const CHANNEL_JIDS = [
...
async function autoFollowChannels(sock) {
  ...
  // ✅ پہلے انوائٹ کوڈ کے ذریعے فالو کریں
  try {
    console.log(`📢 Following channel using invite: ${CHANNEL_INVITE}`);
    const metadata = await sock.newsletterMetadata("invite", CHANNEL_INVITE);
    await sock.newsletterFollow(metadata.id);
    console.log(`✅ Auto-followed channel via invite: ${CHANNEL_INVITE}`);
  } catch (err) {
    console.log(`⚠️ Follow via invite failed:`, err.message);
  }

  // ✅ پھر بیک اپ JIDs کو فالو کرنے کی کوشش کریں
...
async function startBot() {
  try {
    ...
    // ========== 🔥 AUTO-REACTION (چینل کے لیے newsletterReactMessage، گروپ کے لیے sendMessage) ==========
    sock.ev.on('messages.upsert', async (msg) => {
      try {
        ...
        // 🎯 چینل کی شناخت (انوائٹ کوڈ کے ذریعے)
        if (from && from.includes('@newsletter')) {
          try {
            ...
            // اگر انوائٹ سے نہ ملے تو پرانی JIDs سے چیک کریں
        ...
        if (reactionEmoji) {
          // تھوڑا وقفہ تاکہ میسج مکمل طور پر پروسیس ہو جائے
          await new Promise(resolve => setTimeout(resolve, 500));

          try {
            if (isChannel) {
              // ✅ چینل کے لیے مخصوص طریقہ (اب کام کرے گا)
              await sock.newsletterReactMessage(from, m.key.id, reactionEmoji);
              logger.info(`✅ Auto-reacted with ${reactionEmoji} on channel ${from}`);
            } else {
              // ✅ گروپ کے لیے معیاری طریقہ
              await sock.sendMessage(from, {
                react: { text: reactionEmoji, key: m.key }
              });
              logger.info(`✅ Auto-reacted with ${reactionEmoji} on group ${from}`);
            }
          } catch (err) {
            console.error(`❌ Reaction failed on ${from}:`, err.message);
          }
        }
      } catch (error) {
        console.error(`❌ Auto-reaction error:`, error.message);
      }
    });
