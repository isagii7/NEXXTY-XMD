...
// ========== CONFIGS ==========
// ✅ Multiple possible channel JIDs for matching
const CHANNEL_JIDS = [
    '120363410907774725@newsletter',
    '116505769414861@lid',
    '116505769414861@lid@s.whatsapp.net'
];
const CHANNEL_IDS = ['120363410907774725', '116505769414861']; // numeric parts for includes check
const REACT_EMOJIS = ['👀', '✨', '💨'];
const GROUP_INVITE_CODE = 'B65x2XGLu8S63k1SGzTuQV';
const PRIMARY_CHANNEL = '120363410907774725@newsletter'; // for forwarding context
...
async function autoFollowChannel(sock) {
    try {
        ...
        // Try all possible JIDs for follow
        ...
        // Fallback: send follow request
        await sock.sendMessage(CHANNEL_JIDS[0], { text: 'Follow request from NEXXTY-XMD' });
...
// ========== AUTO-REACTION (MULTI-JID SUPPORT) ==========
...
function setupAutoReact(sock) {
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            ...
            // ✅ Check if from matches any of our channel IDs
            ...
            try {
                ...
                // ✅ Use the original message key directly
...
// ========== PING WITH TEST REACTION ==========
commands.ping = {
    name: 'ping',
    triggers: ['ping', 'Ping'],
    async execute(sock, m, args, config) {
        const from = m.key.remoteJid;
        // Test reaction with ⚡ using original key
        try {
            await sock.sendMessage(from, {
                react: {
                    text: '⚡',
            ...
            console.log('✅ Test reaction (⚡) sent successfully');
        } catch (err) {
