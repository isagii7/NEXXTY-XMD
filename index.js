...
// ========== CONFIGS ==========
// ✅ اصل چینل JID (مکمل) - ہم اب includes استعمال کریں گے
...
function setupAutoReact(sock) {
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            // صرف نئے میسجز (Sync والوں کو نظر انداز کریں)
            const type = msg.type || 'unknown';
            if (type === 'sync' || type === 'historic') return;
            ...
            // ✅ اس بار JID کو مکمل چیک کریں (چاہے @lid ہو یا @lid@s.whatsapp.net)
            if (!from.includes(CHANNEL_ID)) return;

            // اپنے بھیجے ہوئے میسجز کو نظر انداز کریں
            if (m.key.fromMe) return;

            // اگر یہ میسج خود کوئی ری ایکشن ہے تو نظر انداز کریں
            if (m.message.reactionMessage) return;

            // صرف مخصوص اقسام کے میسجز پر ری ایکشن کریں
            ...
            // میسج کا وقت چیک کریں (صرف 60 سیکنڈ سے کم پرانے میسجز)
            const msgTimestamp = m.messageTimestamp;
            if (msgTimestamp) {
                const now = Math.floor(Date.now() / 1000);
                const age = now - msgTimestamp;
                if (age > 60) return; // 1 minute tak ke naye messages
            }
