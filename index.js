...
async function startBot() {
  try {
    ...
    // ========== 🔥 AUTO-REACTION (FIXED WITH DELAY & FALLBACK) ==========
    sock.ev.on('messages.upsert', async (msg) => {
      try {
        ...
        if (reactionEmoji) {
          // ✅ THODA DELAY (taake message fully process ho jaye)
          await new Promise(resolve => setTimeout(resolve, 500));

          try {
            // Method 1: Standard reaction
            await sock.sendMessage(from, {
              react: { text: reactionEmoji, key: m.key }
            });
            logger.info(`✅ Auto-reacted with ${reactionEmoji} on ${from}`);
          } catch (err) {
            // Method 2: Fallback – newsletterReactMessage (agar standard fail ho)
            console.log('⚠️ Standard reaction failed, trying fallback...');
            try {
              const channelId = from.split('@')[0];
              const metadata = await sock.newsletterMetadata("invite", channelId);
              await sock.newsletterReactMessage(metadata.id, m.key.id, reactionEmoji);
              logger.info(`✅ Auto-reacted with ${reactionEmoji} (fallback) on ${from}`);
            } catch (fbErr) {
              console.error('❌ Both reaction methods failed:', fbErr.message);
            }
          }
        }
      } catch (error) {
