...
async function startBot() {
    ...
    // ========== MAIN MESSAGE HANDLER WITH DEBUG ==========
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            console.log('📩 Raw message event received'); // <-- یہ لائن لاگ میں آئے گی

            const m = msg.messages[0];
            if (!m) return console.log('❌ No message object');

            // ہر میسج کو لاگ کریں (صرف ڈیبگ کے لیے)
            console.log(`📥 Message from: ${m.key?.remoteJid}`);
            console.log(`📝 Message type: ${m.message ? Object.keys(m.message)[0] : 'unknown'}`);

            if (m.key.fromMe) {
                console.log('⏩ Ignoring own message (fromMe)');
                return;
            }
