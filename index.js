    ...
    try {
        ...
        if (sessionData.noiseKey) {
            ...
            for (const [key, value] of Object.entries(sessionData)) {
                fs.writeFileSync(join(sessionDir, key), typeof value === 'string' ? value : JSON.stringify(value));
            }
...
// ========== 🚀 START BOT (EXACTLY LIKE PAIR.JS) ==========
async function startBot() {
    const { state, saveCreds } = await useMultiFileAuthState(sessionDir);
    const { version } = await fetchLatestBaileysVersion(); // 🔥 Pair.js ki tarah version fetch karo

    const sock = makeWASocket({
        version, // 🔥 Version set karo
        auth: {
            creds: state.creds,
            keys: makeCacheableSignalKeyStore(state.keys, pino({ level: "fatal" })),
        },
        logger: pino({ level: "fatal" }), // 🔥 Pair.js ki tarah logger
        browser: Browsers.windows("Chrome"), // 🔥 Pair.js ki tarah browser
        printQRInTerminal: !SESSION_ID,
        markOnlineOnConnect: false, // 🔥 Pair.js ki tarah
    });
