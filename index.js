...
// ---------- 📥 DOWNLOAD (Updated with siputzx.my.id API) ----------
commands.play = {
    name: 'play',
    triggers: ['play'],
    async execute(sock, m, args, config) {
        ...
        try {
            ...
            const apiUrl = `https://api.siputzx.my.id/api/ytmp3?url=${encodeURIComponent(video.url)}`;
...
commands.ytmp4 = {
    ...
    async execute(sock, m, args, config) {
        ...
        try {
            ...
            const video = search.videos?.[0];
            if (!video) return await sock.sendMessage(from, { text: '❌ No results.' });
            const apiUrl = `https://api.siputzx.my.id/api/ytmp4?url=${encodeURIComponent(video.url)}`;
            const res = await axios.get(apiUrl, { timeout: 30000 });
            const videoUrl = res.data?.data?.url || res.data?.result?.url || res.data?.url;
            if (!videoUrl) throw new Error('No video');
            await sock.sendMessage(from, { video: { url: videoUrl }, caption: `🎬 ${video.title}`, contextInfo: getForwardedContext() }, { quoted: m });
        } catch (err) {
            await sock.sendMessage(from, { text: `❌ ${err.message}` }, { quoted: m });
        }
    }
...
commands.instagram = {
    ...
    async execute(sock, m, args, config) {
        ...
        try {
            const url = args[0];
            const res = await axios.get(`https://api.siputzx.my.id/api/instagram?url=${encodeURIComponent(url)}`, { timeout: 20000 });
            const media = res.data?.data || res.data?.result;
...
commands.tiktok = {
    ...
    async execute(sock, m, args, config) {
        ...
        try {
            const url = args[0];
            const res = await axios.get(`https://api.siputzx.my.id/api/tiktok?url=${encodeURIComponent(url)}`, { timeout: 20000 });
            const media = res.data?.data || res.data?.result;
...
commands.facebook = {
    ...
    async execute(sock, m, args, config) {
        ...
        try {
            const url = args[0];
            const res = await axios.get(`https://api.siputzx.my.id/api/facebook?url=${encodeURIComponent(url)}`, { timeout: 20000 });
            const media = res.data?.data || res.data?.result;
            if (!media?.url) throw new Error('No media');
            await sock.sendMessage(from, { video: { url: media.url }, caption: '📥 Facebook Video', contextInfo: getForwardedContext() }, { quoted: m });
        } catch (err) {
            await sock.sendMessage(from, { text: `❌ ${err.message}` }, { quoted: m });
        }
    }
...
commands.twitter = {
    ...
    async execute(sock, m, args, config) {
        ...
        try {
            const url = args[0];
            const res = await axios.get(`https://api.siputzx.my.id/api/twitter?url=${encodeURIComponent(url)}`, { timeout: 20000 });
            const media = res.data?.data || res.data?.result;
            if (!media?.url) throw new Error('No media');
            await sock.sendMessage(from, { video: { url: media.url }, caption: '📥 Twitter/X Video', contextInfo: getForwardedContext() }, { quoted: m });
        } catch (err) {
            await sock.sendMessage(from, { text: `❌ ${err.message}` }, { quoted: m });
        }
    }
