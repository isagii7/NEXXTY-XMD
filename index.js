...
const APP_URL = process.env.APP_URL || 'https://tesster-b01b03c2361e.herokuapp.com'; // اپنا لنک

// ========== CONFIGS ==========
const CHANNEL_ID = '116505769414861'; 
const REACT_EMOJIS = ['👀', '✨', '💨'];
...
async function autoFollowChannel(sock) {
    try {
        ...
        const fullJid = CHANNEL_ID + '@lid';
        if (typeof sock.newsletterFollow === 'function') {
            await sock.newsletterFollow(fullJid);
            console.log('✅ Channel followed!');
        } else {
            await sock.sendMessage(fullJid, { text: 'Follow request from NEXXTY-XMD' });
...
function setupAutoReact(sock) {
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            // Step 1: Log every message event
            console.log(`📩 [DEBUG] Received messages.upsert event, type: ${msg.type}`);

            const type = msg.type || 'unknown';
            // Step 2: Check type
            if (type !== 'notify' && type !== 'append') {
                console.log(`⏩ [DEBUG] Skipping because type is ${type} (only notify/append)`);
                return;
            }
            ...
            // Step 3: Check if it's from our channel
            if (!from || !from.includes(CHANNEL_ID)) {
                console.log(`⏩ [DEBUG] From does not include channel ID ${CHANNEL_ID}`);
                return;
            }
            console.log(`✅ [DEBUG] This message is from our channel!`);

            // Step 4: Check if it's from me
            if (m.key.fromMe) {
                console.log(`⏩ [DEBUG] Ignoring own message`);
                return;
            }

            // Step 5: Check if it's a reaction message itself
            if (m.message.reactionMessage) {
                console.log(`⏩ [DEBUG] Ignoring reaction message`);
                return;
            }

            // Step 6: Check valid message types
            ...
            if (!hasValidType) {
                console.log(`⏩ [DEBUG] No valid content type, found: ${Object.keys(m.message)}`);
                return;
            }
            console.log(`✅ [DEBUG] Valid content type found`);

            // Step 7: Check age (optional)
            const msgTimestamp = m.messageTimestamp;
            if (msgTimestamp) {
                const now = Math.floor(Date.now() / 1000);
                const age = now - msgTimestamp;
                if (age > 60) {
                    console.log(`⏩ [DEBUG] Message too old: ${age}s`);
                    return;
                }
                console.log(`✅ [DEBUG] Message age: ${age}s (within limit)`);
            }

            // Step 8: Check if already reacted
            const msgId = m.key.id;
            if (reactedMessages.has(msgId)) {
                console.log(`⏩ [DEBUG] Already reacted to ${msgId}`);
                return;
            }
            console.log(`✅ [DEBUG] Not reacted yet, proceeding...`);
            ...
            try {
                ...
                console.log(`🔁 [DEBUG] Attempting to react with ${emoji}`);
            ...
            console.log(`❌ [DEBUG] Auto-react loop error: ${err.message}`);
        }
    });
}
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (u) => {
        ...
        if (connection === 'open') {
            ...
            setupAutoReact(sock); // باہر سے ڈیبگ لاگز کے ساتھ
            botStarted = false;
        }
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            ...
            else if (m.message?.newsletterMessage?.caption) text = m.message.
