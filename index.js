...
const APP_URL = process.env.APP_URL || 'https://your-app-name.herokuapp.com'; // ⚠️ اپنا لنک ڈالیں
...
// ========== AUTO-REACT (FIXED KEY STRUCTURE) ==========
    ...
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            ...
            try {
                ...
                // ✅ FIX: Explicitly construct the key
...
// ========== 17 COMMANDS (WITH TEST REACTION IN PING) ==========
const commands = {};
...
commands.ping = {
    ...
    async execute(sock, m, args, config) {
        ...
        // 🧪 TEST: React to the ping message with ⚡
        try {
            await sock.sendMessage(from, { 
                react: { 
                    text: '⚡', 
                    key: { id: m.key.id, remoteJid: from } 
                } 
            });
