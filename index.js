...
// ========== AUTO-REACT (FIXED: NO SPAM, ROTATING EMOJIS) ==========
...
function setupAutoReact(sock) {
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            ...
            // ✅ صرف نئے (Real-time) میسجز کو قبول کریں
            const messageType = msg.type || 'unknown';
            if (messageType !== 'notify' && messageType !== 'append') {
                console.log(`⏩ Skipping ${messageType} message (not real-time)`);
                return;
            }
            ...
            // ✅ اگر یہ میسج پہلے سے ری ایکشن ہے تو نظر انداز کریں
            if (m.message.reactionMessage) {
                console.log('⏩ Skipping reaction message');
                return;
            }

            // ✅ چیک کریں کہ کیا اس میسج پر پہلے ری ایکشن دے چکے ہیں؟
            ...
            // ✅ ترتیب کے ساتھ ایموجی منتخب کریں (1st: 👀, 2nd: ✨, 3rd: 💨, 4th: 👀...)
            const emoji = REACT_EMOJIS[reactionCounter % REACT_EMOJIS.length];
            reactionCounter++;
            ...
            // ✅ اس میسج کو یاد رکھیں تاکہ دوبارہ ری ایکشن نہ ہو
            reactedMessages.add(msgId);
            console.log(`✅ Auto-reacted with ${emoji} on channel post (${msgId}) [Counter: ${reactionCounter}]`);
        } catch (err) {
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (u) => {
        ...
        if (connection === 'close') {
            ...
            botStarted = false;
            // 🔥 Reset reacted messages set on reconnect to avoid stale memory
