...
// ========== CONFIGS ==========
const CHANNEL_JID = '116505769414861@lid'; // ✅ آپ کی درست JID
const REACT_EMOJIS = ['👀', '✨', '💨'];
...
// ========== AUTO-REACT (FINAL FIX: 1 MESSAGE = 1 REACTION, RANDOM) ==========
...
function setupAutoReact(sock) {
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            // 1. صرف نئے (ریئل ٹائم) میسجز کو قبول کریں
            const type = msg.type || 'unknown';
            if (type !== 'notify' && type !== 'append') {
                return; // پرانے sync والے میسجز کو نظر انداز کریں
            }
            ...
            // 2. چیک کریں کہ یہ چینل کا میسج ہے (آپ کی JID)
            if (from !== CHANNEL_JID) return;

            // 3. اپنے بھیجے ہوئے میسجز کو نظر انداز کریں (تاکہ لوپ نہ بنے)
            if (m.key.fromMe) return;

            // 4. اگر یہ میسج خود کوئی ری ایکشن ہے تو نظر انداز کریں
            if (m.message.reactionMessage) return;

            // 5. صرف مخصوص اقسام کے میسجز پر ری ایکشن کریں (اصل کانٹینٹ والے)
            ...
            // 6. میسج کا وقت چیک کریں (صرف 5 سیکنڈ سے کم پرانے میسجز)
            const msgTimestamp = m.messageTimestamp;
            if (msgTimestamp) {
                const now = Math.floor(Date.now() / 1000);
                const age = now - msgTimestamp;
                if (age > 5) {
                    // console.log(`⏩ Message is ${age}s old (skipping)`);
                    return;
                }
            }

            // 7. کیا ہم پہلے ہی اس میسج پر ری ایکشن دے چکے ہیں؟
            const msgId = m.key.id;
            if (reactedMessages.has(msgId)) {
                // console.log(`⏩ Already reacted to ${msgId}`);
                return;
            }

            // 8. اگر پہلے سے کوئی ری ایکشن چل رہا ہے تو اس کا انتظار کریں (ریٹ لیمٹ سے بچنے کے لیے)
            while (isReacting) {
            ...
            try {
                // 9. بے ترتیب (Random) ایموجی منتخب کریں
                ...
                // 11. اس میسج کو یاد رکھیں تاکہ دوبارہ ری ایکشن نہ ہو
                reactedMessages.add(msgId);
                console.log(`✅ Auto-reacted with ${emoji} on channel post (${msgId})`);

                // 12. ری ایکشن کے بعد 1.5 سیکنڈ کا وقفہ تاکہ ریٹ لیمٹ نہ لگے
                await new Promise(resolve => setTimeout(resolve, 1500));
            } catch (err) {
                // اگر ری ایکشن ناکام ہو (مثلاً ریٹ لیمٹ لگ جائے) تو صرف لاگ کریں، اسپام نہ کریں
            ...
            // 13. Set بہت بڑا ہونے پر پرانے IDs کو صاف کریں (میموری کے لیے)
            if (reactedMessages.size > 1000) {
                const toDelete = [...reactedMessages].slice(0, 500);
                toDelete.forEach(id => reactedMessages.delete(id));
                console.log(`🧹 Cleaned old reaction records. Size: ${reactedMessages.size}`);
            }
        } catch (err) {
            // عمومی خرابی کو نظر انداز کریں تاکہ بوٹ کریش نہ ہو
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (u) => {
        ...
        if (connection === 'close') {
            ...
            botStarted = false;
            reactedMessages.clear(); // میموری صاف کریں
    ...
    sock.ev.on('messages.upsert', async (msg) => {
        try {
            ...
            else if (m.message?.newsletterMessage?.ca
